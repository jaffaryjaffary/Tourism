...
import {  FetchApprovedVisitorByIdAction, SearchProfileAction } from "../app/Actions";
...
export default function LiveSearchTablePaginationPage() {
  ...
  useEffect(() => {
    startTransition(async () => {
      const data = await SearchProfileAction(keyword, page);
      setUsers(data.users);
  ...
  async function HandleUserDetails(currentId) {
    ...
    router.push(`/ViewUserDetails/${currentId}`)
    
  }

  // async function HandleDelete(currentId) {
  //   await DeleteUserAction(currentId,'/Admin_Dashboard')
  ...
  return (
    <div className="overflow-x-scroll bg-white shadow-lg p-3">
        ...
        <div className="lg:flex justify-between items-center mt-5 p-4">
            ...
            <h2 className="text-xl font-bold">Contact Visitors Information</h2>
       <div>
                  <input
        type="text"
